...
public class ChatActivity extends Activity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		userAccount = account;
		tvFriendAccount = (TextView) this.findViewById(R.id.chat_contact_name);
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				blacklistAccountList = blacklistDBManager
						.blacklistAllAccountQuery(db, account);
				db.close();
						...
						public void run() {
							try {
								...
								MessageEntity messageEntity = new MessageEntity(
										otherAccount, account, text, "readed",
										messageDate);
								MessageDBManager.messageInsert(db,
										messageEntity);
								db.close();
		...
		// 初始化聊天界面
		ArrayList<HashMap<String, String>> messageReceive = MessageDBManager
				.messageQueryOfSender(db, otherAccount);
		ArrayList<HashMap<String, String>> messageSend = MessageDBManager
				.messageQueryOfUser(db, otherAccount);
		...
		for (int i = 0; i < messageAll.size(); i++) {
			for (int j = 0; j < userNumber.size(); j++) {
				if (userNumber.get(j).equals(messageAll.get(i).get("number"))) {
					addInfoToList(messageAll.get(i).get("message"), USER,
							messageAll.get(i).get("time"));
				}
			}
			for (int j = 0; j < otherNumber.size(); j++) {
				if (otherNumber.get(j).equals(messageAll.get(i).get("number"))) {
					addInfoToList(messageAll.get(i).get("message"), OTHER,
							messageAll.get(i).get("time"));
				}
			}
		}
		...
		fileTransferManager.addFileTransferListener(new FileTransferListener() {
			public void fileTransferRequest(FileTransferRequest request) {
				...
				try {
					String userDire = ChatActivity.this.getApplication()
							.getExternalFilesDir(null).getPath()
							+ "/user/" + account + "/";
					transfer.recieveFile(new File(userDire
					...
					MessageEntity messageEntity = new MessageEntity(account,
							otherAccount, "接收文件：" + transfer.getFileName(),
							"readed", messageDate);
					MessageDBManager.messageInsert(db, messageEntity);
					db.close();
					addInfoToList("接收文件" + transfer.getFileName(), OTHER,
							messageDate);
					handler.post(new Runnable() {
						@Override
						public void run() {
				...
				// File("shakespeare_complete_works.txt"));
				// } else {
				// // Reject it
	...
	// @Override
	// public void run() {
	// while(!outgoingFileTransfer.isDone()) { //可以直接加到 transfer.sendFile 或者
	// transfer.recieveFile 后面。
					...
					if (!blacklistAccountList.contains(sender)) {
						if (otherAccount.equals(sender)) {
							...
							if (message.getBody() == null
								...
								String messageDate = GetNetWorkTime
										.getWebsiteDatetime();
								addInfoToList(message.getBody(), OTHER,
										messageDate);
								handler.post(new Runnable() {
									@Override
									public void run() {
				...
				public void run() {
					try {
						List<UserSearchEntity> listInfoFriend = UsersSearch
								.searchUsers(connect, serverAddress,
										otherAccount);
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (who == 0) {
				...
				holder.tvText.setText(chatList.get(position).get("text")
						.toString());
				holder.tvDate.setText(chatList.get(position).get("time")
						.toString());
				return convertView;
			} else {
				convertView = LayoutInflater.from(context).inflate(
